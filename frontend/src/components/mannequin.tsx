...
import { Activity, ScanFace, Target, Database, Zap, Brain, Sparkles } from "lucide-react";
...
import FemaleBodySvg from "@/assets/female-vector.svg?react";
import MaleBodySvg from "@/assets/male-vector.svg?react";
...
export default function BodySymptomSelector({ age, gender, onComplete }: BodySymptomSelectorProps) {
    ...
    const filteredResults = useMemo(() => {
        ...
        SYSTEMIC_SYMPTOMS.forEach(s => {
            ...
            if (translated.toLowerCase().includes(query)) {
                uniqueMatches.set(s.label, translated);
            }
        });
                ...
                <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-primary text-[10px] font-black tracking-[0.5em] uppercase">
                                {t("Insights")} ({age} {t("years")}, {t(gender.toUpperCase())})
                            </span>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-center">
                        ...
                        <div className="flex bg-secondary-foreground/40 p-1 rounded-xl border border-secondary/5 backdrop-blur-md">
                            <button onClick={() => { setView("physical"); resetToDefault(); }} className={`px-6 py-2 rounded-lg text-[10px] font-black tracking-widest transition-all cursor-pointer ${view === "physical" ? "bg-primary text-secondary-foreground shadow-lg" : "text-secondary/40 hover:text-secondary"}`}>{t("PHYSICAL SCAN")}</button>
                            <button onClick={() => { setView("systemic"); resetToDefault(); }} className={`px-6 py-2 rounded-lg text-[10px] font-black tracking-widest transition-all cursor-pointer ${view === "systemic" ? "bg-primary text-secondary-foreground shadow-lg" : "text-secondary/40 hover:text-secondary"}`}>{t("GENERAL SYMPTOMS")}</button>
                        </div>
                    </div>
                </div>
                            ...
                            {hoveredPart && (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute bottom-6 left-8 z-30 flex flex-col items-start font-mono">
                                    <div className="flex items-center gap-2 mb-1">
                                        <div className="w-1.5 h-1.5 bg-primary animate-pulse rounded-full shadow-[0_0_8px_var(--primary)]" />
                        ...
                        <button
                            ...
                            className={`w-full py-4 rounded-2xl font-black tracking-widest text-xs flex items-center justify-center gap-2 transition-all duration-300 ${
                                activeSymptomsCount > 0
                                    ? "bg-gradient-to-r from-primary to-purple-600 text-secondary-foreground shadow-[0_0_25px_rgba(168,85,247,0.4)] hover:shadow-[0_0_35px_rgba(168,85,247,0.6)] cursor-pointer hover:scale-[1.01]"
                                    : "bg-secondary/10 text-secondary/30 cursor-not-allowed"
                            }`}
                        >
                            <Sparkles className="w-4 h-4" />
                            {t("GENERATE DEFICIENCY REPORT")}
                        </button>
                    </div>
                </div>
            </div>
