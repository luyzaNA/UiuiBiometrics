        ...
        <div className="max-w-4xl mx-auto px-0 md:px-6 py-4 md:py-10 animate-fadeIn bg-background">
            ...
            <div className="mb-12">
                ...
                <p className="text-sm md:text-base text-muted-foreground font-light max-w-2xl leading-relaxed">
                    ...
                    <span className="font-medium text-foreground">{menuData?.reviewAfterDays || 30} {t("days")}</span>.
                </p>
            </div>

            <div className="space-y-10">
                {menuData?.deficiencyTargets?.map((target: any, index: number) => (
                    <section
                        key={index}
                    ...
                    >
                        ...
                        <div className="border-b border-foreground/10 pb-4 mb-2">
                            ...
                            <h2 className="text-2xl md:text-3xl font-medium text-foreground capitalize tracking-tight">
                                {getLocalizedText(target.deficiencyName)}
                            </h2>
                        </div>

                        <div className="flex flex-col">
                            {target.recommendedFoods?.map((food: any, fIndex: number) => (
                                <div
                                    key={fIndex}
                                ...
                                >
                                    <div className="md:col-span-6 lg:col-span-7 flex flex-col justify-center">
                                        <h3 className="text-base md:text-lg font-medium text-foreground group-hover:text-primary transition-colors duration-200">
                                            {getLocalizedText(food.foodName)}
                                        </h3>

                                        {food.absorptionBoosters?.length > 0 && (
                                            <div className="flex items-start gap-1.5 text-xs text-muted-foreground/70 font-light mt-1">
                                                ...
                                                <span className="leading-snug">
                                                    ...
                                                    <span className="font-medium text-foreground/70 lowercase">
                                                        {food.absorptionBoosters.map((b: any) => getLocalizedText(b)).join(", ")}
                                                    </span>
                                                    {" "}
