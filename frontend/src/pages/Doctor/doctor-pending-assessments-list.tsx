...
import { Clock, ChevronRight, ClipboardList, Stethoscope, AlertCircle } from "lucide-react";
...
export default function PendingAssessmentsList() {
    ...
    if (isLoading) {
        return (
            <div className="flex justify-center items-center h-48 border border-secondary/10 rounded-2xl bg-background">
    ...
    if (assessments.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-10 text-center border border-dashed border-secondary/20 rounded-2xl bg-background min-h-screen">
                ...
                <p className="text-sm text-secondary/70 mt-2">
                    {t("All patient assessments have been reviewed.")}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-5 min-h-screen p-10">
            <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
                    <Stethoscope className="w-5 h-5 text-primary" />
                    {t("Pending reviews")}
                </h2>
                <span className="bg-secondary/30 text-secondary-foreground text-xs font-medium px-3 py-1 rounded-full">
